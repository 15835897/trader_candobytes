...
#FtdcSyncFlagType是一个同步标记类型
THOST_FTDC_SYNF_No = '1' #未同步
THOST_FTDC_SYNF_Yes = '0' #已同步
#FtdcSettArchiveStatusType是一个结算确认数据归档状态类型
...
#FtdcSpecProductTypeType是一个特殊产品类型类型
THOST_FTDC_SPT_IneForeignCurrency = '2' #能源中心货币质押产品
...
#FtdcDAClientTypeType是一个资产管理客户类型类型
...
#FtdcCurrencySwapStatusType是一个申请状态类型
...
#FtdcMortgageFundUseRangeType是一个货币质押资金可用范围类型
...
#FtdcCloseDealTypeType是一个平仓处理类型类型
...
#FtdcCusAccountTypeType是一个结算账户类型类型
THOST_FTDC_CAT_Assetmgr = '2' #资管结算账户
...
#FtdcSwapSourceTypeType是一个换汇数据来源类型
...
#FtdcMaxMarginSideAlgorithmType是一个大额单边保证金算法类型
...
#FtdcFBEAlreadyTradeType是一个换汇已交易标志类型
...
#FtdcFBEUserEventTypeType是一个银期换汇用户事件类型类型
...
#FtdcFBEExchStatusType是一个换汇交易状态类型
...
#FtdcCheckInstrTypeType是一个合约比较类型类型
...
#FtdcReportStatusType是一个报表数据生成状态类型
...
#FtdcMarginRateTypeType是一个冲突保证金率类型类型
...
#FtdcFundMortDirectionType是一个货币质押方向类型
...
#FtdcCurrExDirectionType是一个换汇类型类型
THOST_FTDC_CED_Sale = '1' #售汇
THOST_FTDC_CED_Settlement = '0' #结汇
#FtdcExprSetModeType是一个日期表达式设置类型类型
...
#FtdcBackUpStatusType是一个备份数据状态类型
...
#FtdcClientRegionType是一个开户客户地域类型
...
#FtdcUOAAssetmgrTypeType是一个投资类型类型
...
#FtdcFundMortgageTypeType是一个货币质押类型类型
...
#FtdcDBOperationType是一个记录操作类型类型
...
#FtdcBusinessClassType是一个换汇类别类型
...
#FtdcLanguageTypeType是一个通知语言类型类型
