...
class TraderApi:
    @staticmethod
    def CreateTraderApi(FlowPath=""):
        if FlowPath:
            FlowPath=os.path.abspath(FlowPath)
        api_ptr=_ctp_Trader.create_TraderApi(FlowPath)
    ...
    def ReqAuthenticate(self, pReqAuthenticate, nRequestID):
        '''客户端认证请求'''
        return _ctp_Trader.ReqAuthenticate(self.api_ptr, pReqAuthenticate, nRequestID)
...
@remark 不再使用本接口对象时,调用该函数删除接口对象'''
    ...
    def ReqQryInvestorProductGroupMargin(self, pQryInvestorProductGroupMargin, nRequestID):
        '''请求查询投资者品种/跨品种保证金'''
        return _ctp_Trader.ReqQryInvestorProductGroupMargin(self.api_ptr, pQryInvestorProductGroupMargin, nRequestID)
...
@remark RegisterNameServer优先于RegisterFront'''
        return _ctp_Trader.RegisterNameServer(self.api_ptr, pszNsAddress)
    ...
    def ReqQryExchangeRate(self, pQryExchangeRate, nRequestID):
        '''请求查询汇率'''
        return _ctp_Trader.ReqQryExchangeRate(self.api_ptr, pQryExchangeRate, nRequestID)
...
@return 线程退出代码'''
    ...
    def ReqQryExchangeMarginRate(self, pQryExchangeMarginRate, nRequestID):
        '''请求查询交易所保证金率'''
        return _ctp_Trader.ReqQryExchangeMarginRate(self.api_ptr, pQryExchangeMarginRate, nRequestID)

    def ReqQryProduct(self, pQryProduct, nRequestID):
        '''请求查询产品'''
        return _ctp_Trader.ReqQryProduct(self.api_ptr, pQryProduct, nRequestID)
...
@remark 初始化运行环境,只有调用后,接口才开始工作'''
    ...
    def ReqUserLogin(self, pReqUserLogin, nRequestID):
        '''用户登录请求'''
        return _ctp_Trader.ReqUserLogin(self.api_ptr, pReqUserLogin, nRequestID)
...
@remark “tcp”代表传输协议，“127.0.0.1”代表服务器地址。”17001”代表服务器端口号。'''
    ...
    def ReqQryExchangeMarginRateAdjust(self, pQryExchangeMarginRateAdjust, nRequestID):
        '''请求查询交易所调整保证金率'''
        return _ctp_Trader.ReqQryExchangeMarginRateAdjust(self.api_ptr, pQryExchangeMarginRateAdjust, nRequestID)
...
@remark 只有登录成功后,才能得到正确的交易日'''
        return tou(_ctp_Trader.GetTradingDay(self.api_ptr, ))
    ...
    def ReqQueryCFMMCTradingAccountToken(self, pQueryCFMMCTradingAccountToken, nRequestID):
        '''请求查询监控中心用户令牌'''
        return _ctp_Trader.ReqQueryCFMMCTradingAccountToken(self.api_ptr, pQueryCFMMCTradingAccountToken, nRequestID)

    def RegisterFensUserInfo(self, pFensUserInfo):
        '''注册名字服务器用户信息
@param pFensUserInfo：用户信息。'''
        return _ctp_Trader.RegisterFensUserInfo(self.api_ptr, pFensUserInfo)
...
@remark 该方法要在Init方法前调用。若不调用则不会收到私有流的数据。'''
    ...
    def ReqQrySecAgentACIDMap(self, pQrySecAgentACIDMap, nRequestID):
        '''请求查询二级代理操作员银期权限'''
        return _ctp_Trader.ReqQrySecAgentACIDMap(self.api_ptr, pQrySecAgentACIDMap, nRequestID)
